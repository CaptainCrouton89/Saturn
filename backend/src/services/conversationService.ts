 ...
 *
 * Orchestrates conversation flow using AI SDK streamText() and manages
 * conversation state in PostgreSQL. Integrates with:
 * - AI SDK orchestrator (agents/orchestrator.ts) for response generation
 * - Background worker (via memoryQueue) for async entity extraction
 * - Summary service for conversation summarization
 *
 * Message Flow (AI SDK):
 * 1. Fetch existing transcript (StoredMessage[]) from PostgreSQL
 * 2. Pass to orchestrator which converts to CoreMessage for AI SDK
 * 3. orchestrator returns StoredMessage[] (no conversion needed)
 * 4. Store directly in PostgreSQL content_raw field
 *
 * AI SDK Implementation:
 * - Uses streamText() for multi-turn conversations with tool support
 * - StoredMessage format matches AI SDK CoreMessage structure
 * - Tools are AI SDK tools defined with tool() helper
 * - Error handling simplified (no graph-specific errors)
 */
...
export class ConversationService {
  ...
  async createConversation(
    ...
    return withSpan('conversation.create', { userId }, async () => {
      const supabase = supabaseService.getClient();
   ...
   * Uses AI SDK streamText() for conversation generation
   */
  async processExchange(
    ...
    // Get existing transcript from Supabase - it stores messages in simple JSON format
    const existingTranscript = (source.content_raw as unknown as StoredMessage[]) ?? [];

    // Check if this is an onboarding conversation (no longer stored in source table - would need separate tracking if needed)
    const isOnboarding = false;

    // Run AI SDK agent with user message
    ...
    // fullMessages is already in StoredMessage format (AI SDK compatible)
    // Update source with new transcript (cast to Json for database)
    const { error: updateError } = await supabase
      .from('source')
      .update({
        content_raw: fullMessages as unknown as never,
      })
      .eq('id', conversationId);
    ...
    // Convert StoredMessage to ConversationTurn format for API response
    // Only include user and assistant messages (not tool messages)
    const conversationTurns: ConversationTurn[] = fullMessages
      .filter(msg => msg.role === 'human' || msg.role === 'ai')
      .map(msg => ({
        speaker: msg.role === 'human' ? 'user' : 'assistant',
        message: msg.content,
  ...
  async endConversation(
    ...
    return withSpan('conversation.end', { userId, conversationId }, async () => {
      ...
      const transcript = existingSource.content_raw as unknown as StoredMessage[];
      const messageCount = transcript ? transcript.length : 0;
      const startTime = existingSource.started_at
        ? new Date(existingSource.started_at)
        : existingSource.created_at
        ? new Date(existingSource.created_at)
        : new Date();
      const endTime = new Date();
      const durationSeconds = Math.round((endTime.getTime() - startTime.getTime()) / 1000);

      // Add duration and message count to current span context
      console.log(`[Conversation End] conversationId=${conversationId}, duration=${durationSeconds}s, messages=${messageCount}`);
      ...
      // Wrap this in a separate span to track job queueing
      try {
        await withSpan('enqueue-memory-extraction', {
          userId,
          conversationId,
          messageCount,
        }, async () => {
